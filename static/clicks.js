    ...
    try {
        ...
        if (saved) {
            ...
            gameState = {
                ...
                clicksPerSecond: parsed.clicksPerSecond || 0
            };
            return true;
        }
    } catch (error) {
...
async function initClicks() {
    ...
    const loadedFromStorage = loadGameState();
    
    if (!loadedFromStorage) {
        console.log('[Init] No saved state found, loading from backend...');
        // If no saved state, load from backend
        try {
            const state = await getGameState();
            updateGameState(state);
            console.log('[Init] Loaded from backend:', state);
        } catch (error) {
            console.error('[Init] Failed to load initial game state:', error);
            console.log('[Init] Using default game state');
        }
    } else {
        console.log('[Init] Loaded from localStorage:', gameState);
    ...
    console.log('[Init] Click handler registered');

    // Calculate CPS every second
    setInterval(calculateCPS, 1000);
    
    // Save state every 5 seconds (auto-save)
    setInterval(saveGameState, 5000);
    
    console.log('[Init] Game fully initialized!');
}
...
async function handleClick(event) {
    try {
        ...
        console.log('[Click] Sending click to backend...');
        
        // Send click to backend
        const response = await sendClick();
        
        console.log('[Click] Backend response:', response);
        ...
        if (response.status === 'success') {
            ...
            console.error('[Click] Unexpected response status:', response);
            showError('Unexpected server response');
        }
    } catch (error) {
        console.error('[Click] Error:', error);
        showError('Failed to record click - is the backend running?');
    }
}
